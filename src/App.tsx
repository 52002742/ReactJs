...
function App() {
  ...
  const filterData = (datarray: any[], query: any) => {
    return datarray.filter((country) =>
      country?.name?.toLowerCase().includes(query?.toLowerCase())
    );
  };

  const fetchData = async (srcTxt: any) => {
    const { data } = await axios.get("/country.json");

    console.log("datadatadata", data);
    const filterdData = data.filter((country: { name: string }) =>
      country?.name?.toLowerCase().includes(srcTxt?.toLowerCase())
    );
    console.log("fetch", filterdData);
    setIsShow(false);
